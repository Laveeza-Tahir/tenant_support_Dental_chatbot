...
# Path: app/core/workflows/dental_workflow.py
...
class DentalWorkflow:
    ...
    async def run(self, session_id: str, user_message: str):
        ...
        # 1. Check if appointment was just completed
        if state.pop("just_booked", False):
            # User just booked – start fresh with completely clean state
            # Clear any appointment-related flags to prevent lingering state issues
            state.pop("pending_intent", None)
            state.pop("intent", None)
            state.pop("awaiting", None)
            
            # Clear any slot values
            for slot in ["patient_name", "appointment_date", "appointment_time"]:
                state.pop(slot, None)
                
            # Now add a welcome message
            state["user_message"] = "What else can I help you with?"
            state = await self.input(state)
            state = await self.classifier(state)
            intent = state.get("intent")
            
            # Log state for debugging
            print(f"After appointment completion, new state: {state}")
        else:
            # 2. Handle awaiting inputs (appointment, etc.)
            awaiting = state.get("awaiting")
            if awaiting == "patient_name":
                state["patient_name"] = user_message
                state["awaiting"] = None
            ...
            # 3. Process input and classification
            state = await self.input(state)
            state = await self.classifier(state)
        ...
        # Additional check for location-related terms to override other intents
        user_message = state.get("user_message", "").lower()
        location_terms = ["location", "address", "where", "office", "directions", "clinic", "map"]
        if any(term in user_message for term in location_terms):
            print(f"Location term detected in message: '{user_message}', overriding intent to contact_info")
            intent = "contact_info"
            state["intent"] = "contact_info"
            # Clear any pending intents to ensure location gets handled
            state.pop("pending_intent", None)
        
        # Log the detected intent
        print(f"Detected intent: {intent}, user message: {state.get('user_message')}")

        # Check if user is starting a new appointment request after completing one
        if intent == "appointment" and not state.get("awaiting") and not state.get("pending_intent"):
            # Clear any lingering appointment data to start fresh
            for slot in ("patient_name", "appointment_date", "appointment_time"):
                state.pop(slot, None)

        handled = False
        
        # Special handling for contact info/location queries - prioritize these
        ...
        # Appointment flow
            ...
            if state.get("intent") == "appointment_completed":
                ...
                for slot in ("patient_name", "appointment_date", "appointment_time"):
                    state.pop(slot, None)
                state["just_booked"] = True  # Flag that appointment was booked
            handled = True
